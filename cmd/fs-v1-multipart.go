...
func (fs *FSObjects) backgroundAppend(ctx context.Context, bucket, object, uploadID string) {
	...
	if file == nil {
		file = &fsAppendFile{
			...
			filePath: pathJoin(fs.getbgAppendsPath(bucket, object, uploadID), fmt.Sprintf("%s.%s", uploadID, mustGetUUID())),
		}
...
func (fs *FSObjects) patchPart(partID int, r *PutObjReader, file *fsAppendFile, data *hash.Reader, offset int64) (pi PartInfo, e error) {
	file.Lock()
	defer file.Unlock()
	duration := time.Duration(globalFileOpenTimeout) * time.Minute
...
func (fs *FSObjects) CompleteMultipartUpload(ctx context.Context, bucket string, object string, uploadID string, parts []CompletePart, opts ObjectOptions) (oi ObjectInfo, e error) {
	...
	appendFilePath := pathJoin(fs.getbgAppendsPath(bucket, object, uploadID), fmt.Sprintf("%s.%s", uploadID, mustGetUUID()))
	...
	// Purge multipart folders
	{
		...
		tmpDataPath := filepath.Dir(fs.getbgAppendsPath(bucket, object, uploadID))
